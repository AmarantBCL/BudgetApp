...
//        val headerColor = if (isSubtract) ContextCompat.getColor(requireContext(), R.color.negative_red)
//            else ContextCompat.getColor(requireContext(), R.color.positive_green)
...
//        binding.lblHeader.setTextColor(headerColor)
        ...
        binding.tvCurrentAmount.text = currencySymbol + " " + saved
        binding.tvTargetAmount.text = currencySymbol + " " + target
        binding.btnAddEntry.text = if (isSubtract) "Take Money" else "Add Money"
    ...
    private fun calculatePreliminarySavings(enteredAmount: String) {
        val amount = enteredAmount.ifEmpty { "0" }.toDouble()
        updatedBalance = currentBalance
        if (isSubtract) {
